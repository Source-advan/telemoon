...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == "لیست تحریم" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'تحریم' then-- /ban
  ...
  if matches[1]:lower() == 'حذف تحریم' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'اخراج' then
  ...
  if matches[1]:lower() == 'تحریم گلوبال' and is_admin1(msg) then -- Global ban
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'یوزر ['..user_id..' ] تحریم گلوبال شد'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
  ...
  if matches[1]:lower() == 'حذف تحریم گلوبال' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == "لیست تحریم گلوبال" then -- Global ban list
    return banall_list()
  end
end
